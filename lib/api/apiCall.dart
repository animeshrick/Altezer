...
class Networkcall {
 ...
 ///  ---------------------- edit bank detals ---------------
  ...
  Future<bool?> getUpdateUserInfoAPICall(
    ...
    var resp = response.body;
    final myResponse = jsonDecode(resp);
    // print('update profile -- $updateUserInfo');
    // print(' $data --- $resp');
    try {
      if (response.statusCode == 200) {
        if (myResponse['status'] == success) {
